...
class MAIN:
    ...
    def __init__(self):
        ...
        try:
            self.LOGO()
            printf(Panel(f"[bold blue]Please fill in your Instagram account details such as username and password, use `[bold Red]:[bold blue]` as a\nseparator, you must also use a fake account to log in!", width=59, style="bold bright_black", title="[bold bright_black][LOGIN 🔗]", subtitle="[bold bright_black]╭──────", subtitle_align="left"))
            ...
            if ':' in str(self.ACCOUNTS):
                ...
                printf(Panel(f"[bold white]Please fill in your Instagram account username, make sure the account is not locked and the\nusername is correct. Example:[bold green] @nyxtoffical [bold Red]public account are only working", width=59, style="bold bright_black", title="[bold bright_black][Username]", subtitle="[bold bright_black]╭──────", subtitle_align="left"))
...
[bold white]Jumlah :[bold yellow] {self.JUMLAH}""", width=59, style="bold bright_black", title="[bold bright_black][success]"))
                ...
                printf(Panel(f"[bold red]You did not fill in the account data correctly, make sure the username and password\nseparator is a colon, please try again!", width=59, style="bold bright_black", title="[bold bright_black][Permission Denied ❌]"))
                sys.exit()
        except (Exception) as e:
...
█ █░░░█ ███░ ░█░ █░░█ ████ █░░░█ ░██░ ███░ █░░█  
        ...
        [underline yellow]Free Instagram Followers - by Nyxt""", width=59, style="bold bright_black"))
        return (True)
    ...
    def DELAY(self, menit, detik, your_username):
        ...
        while (self.TOTAL):
            ...
            printf(f"[bold bright_black]   ──>[bold green] @{str(your_username)[:20].upper()}[bold white]/[bold green]{MENIT:02d}:{DETIK:02d}[bold white] SUCCESS:-[bold green]{len(SUKSES)}[bold white] GAGAL:-[bold red]{len(GAGAL)}     ", end='\r')
            time.sleep(1)
            self.TOTAL -= 1
        return (True)
...
if __name__ == '__main__':
    try:
        if os.path.exists("Official/Subscribe.json") == False:
            youtube_url = json.loads(requests.get('').text)['Link']
            os.system(f'xdg-open {youtube_url}')
            with open('Official/Subscribe.json', 'w') as w:
                w.write(json.dumps({
                    "Status": True
                }))
            w.close()
